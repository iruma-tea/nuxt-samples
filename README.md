...
## エラーを発生させる
- createError()  
引数としては、エラー内容を表す文字列を渡す。このエラーオブジェクトをthrowすることで、その時点でエラーが発生するようになっている。   
- NuxtErrorBoundaryタグ
子コンポーネントでエラーが発生した場合にエラー処理に利用されるのが、*NuxtErrorBoundary*タグ。  
子コンポーネントをレンダリングするタグを*NuxtErrorBundary*タグで囲み。さらにコンポーネントでエラーが発生したさいの表示内容を記述する   
そのエラー表示タグにv-slot:errorディレクティブを記述する。この属性値のプロパティにerrorエラー内容が格納され表示することが可能。